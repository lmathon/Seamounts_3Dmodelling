...
load("05_Marine_Spatial_Planning/03_Prioritization/Prioritization30/Solution_blm_0/sol_2d_data.rdata")

pu_habitat <- as.data.frame(pu_habitat)
sol_2d_data <- as.data.frame(sol_2d_data)
sol_2d_data$geometry <- as.character(sol_2d_data$geometry)
pu_habitat$geometry <- as.character(pu_habitat$geometry)
...
# surface selectionnee pente = 16362
surf_slope_select <- sol_2d_habitat_select %>%
  filter(Habitat=="DeepSlope") %>%
  nrow()

# surface selectionnee seamount = 13426
surf_seamount_select <- sol_2d_habitat_select %>%
  filter(Habitat=="Seamount") %>%
  nrow()

perc_SM <- 13426/29788*100
perc_DS <- 16362/29788*100
