...
abund_0_200 <- abund_0_200 %>% dplyr::select(x,y,1:23)
...
abund_200_400 <- abund_200_400 %>% dplyr::select(x,y,1:23)
...
abund_400_600 <- abund_400_600 %>% dplyr::select(x,y,1:23)
...
load("04_Modelling/01_benthic/02_eDNA/01_BRT_richness_eDNA/BRT_Output_edna/benthic_motu_predict.rdata")
...
##################################################################################################################################################
## eDNA reads MOTUs benthic

load("04_Modelling/01_benthic/02_eDNA/02_GJAM_eDNA/GJAM_Output_edna/predictions.rdata")
...
reads_0_200 <- reads_0_200 %>% dplyr::select(x,y,1:12)
...
reads_200_400 <- reads_200_400 %>% dplyr::select(x,y,1:12)
...
reads_400_600 <- reads_400_600 %>% dplyr::select(x,y,1:12)
...
load("04_Modelling/02_pelagic/02_eDNA/01_BRT_richness_eDNA/BRT_Output_edna/pelagic_motu_predict.rdata")
...
##################################################################################################################################################
## eDNA reads MOTUs Pelagic

load("04_Modelling/02_pelagic/02_eDNA/02_GJAM_eDNA/GJAM_Output_edna/predictions.rdata")

predictions <- predictions %>%
  filter(!is.na(Sampling_Depth))

# 0-200
reads_pel_0_200 <- predictions %>%
  filter(SamplingDepth <= 200)

reads_pel_0_200 <- reads_pel_0_200 %>% dplyr::select(x,y,1:10)
reads_pel_0_200$x <- as.factor(reads_pel_0_200$x)
reads_pel_0_200$y <- as.factor(reads_pel_0_200$y)

reads_pel_0_200 <- reads_pel_0_200 %>%
  group_by(x,y) %>%
  summarise_all(funs(sum))


# 200-400
reads_pel_200_400 <- predictions %>%
  filter(SamplingDepth <= 400) %>%
  filter(SamplingDepth > 200)

reads_pel_200_400 <- reads_pel_200_400 %>% dplyr::select(x,y,1:10)
reads_pel_200_400$x <- as.factor(reads_pel_200_400$x)
reads_pel_200_400$y <- as.factor(reads_pel_200_400$y)

reads_pel_200_400 <- reads_pel_200_400 %>%
  group_by(x,y) %>%
  summarise_all(funs(sum))


# 400-600
reads_pel_400_600 <- predictions %>%
  filter(SamplingDepth > 400)

reads_pel_400_600 <- reads_pel_400_600 %>% dplyr::select(x,y,1:10)
reads_pel_400_600$x <- as.factor(reads_pel_400_600$x)
reads_pel_400_600$y <- as.factor(reads_pel_400_600$y)

reads_pel_400_600 <- reads_pel_400_600 %>%
  group_by(x,y) %>%
  summarise_all(funs(sum))
...
df_0_200 <- left_join(pelagic_motu_0_200, bruvs_richness_0_200, by=c("x","y"))
df_0_200 <- left_join(df_0_200, bruvs_biomass_0_200, by=c("x","y"))
df_0_200 <- left_join(df_0_200, bruvs_abundance_0_200, by=c("x","y"))
df_0_200 <- left_join(df_0_200, abund_0_200, by=c("x","y"))
...
df_0_200 <- left_join(df_0_200, reads_pel_0_200, by=c("x","y"))
...
df_200_400 <- left_join(pelagic_motu_200_400, bruvs_richness_200_400, by=c("x","y"))
df_200_400 <- left_join(df_200_400, bruvs_biomass_200_400, by=c("x","y"))
df_200_400 <- left_join(df_200_400, bruvs_abundance_200_400, by=c("x","y"))
df_200_400 <- left_join(df_200_400, abund_200_400, by=c("x","y"))
...
df_200_400 <- left_join(df_200_400, reads_pel_200_400, by=c("x","y"))
...
df_400_600 <- left_join(pelagic_motu_400_600, bruvs_richness_400_600, by=c("x","y"))
df_400_600 <- left_join(df_400_600, bruvs_biomass_400_600, by=c("x","y"))
df_400_600 <- left_join(df_400_600, bruvs_abundance_400_600, by=c("x","y"))
df_400_600 <- left_join(df_400_600, abund_400_600, by=c("x","y"))
...
df_400_600 <- left_join(df_400_600, reads_pel_400_600, by=c("x","y"))
...
df_0_200 <- df_0_200[,-55]
df_200_400 <- df_200_400[,-55]
df_400_600 <- df_400_600[,-55]
