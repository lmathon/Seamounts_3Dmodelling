...
formula <- as.formula(~ SSTmean + I(SSTmean^3) + BottomDepth + I(BottomDepth^2) +
                        ...
                        LandMinDist + I(LandMinDist^2) + Salinity + I(Salinity^2) +
                        TravelTime + Habitat + SummitRugosity)
...
save(gjam, file="04_Modelling/01_benthic/01_BRUVs/04_GJAM_abundance_BRUVS/GJAM_Output_bruvs/gjam_model.rdata")
...
ggsave(plot_sens, file="04_Modelling/01_benthic/01_BRUVs/04_GJAM_abundance_BRUVS/GJAM_Output_bruvs/gjamOutput/plot_sens.png", width=6, height = 5)
...
new_df <- df_benthic %>%
  dplyr::select(x, y, Habitat,Salinity,EastwardVelocity,SSTmean,BottomDepth,LandMinDist,TravelTime,SummitRugosity)

new_df$Habitat <- as.factor(new_df$Habitat)

new_df$`I(Salinity^2)` <- (new_df$Salinity)^2
new_df$`I(BottomDepth^2)` <- (new_df$BottomDepth)^2
new_df$`I(LandMinDist^2)` <- (new_df$LandMinDist)^2
new_df$`I(EastwardVelocity^3)` <- (new_df$EastwardVelocity)^3
new_df$`I(SSTmean^3)` <- (new_df$SSTmean)^3
...
save(predictions, file="04_Modelling/01_benthic/01_BRUVs/04_GJAM_abundance_BRUVS/GJAM_Output_bruvs/predictions.rdata")
