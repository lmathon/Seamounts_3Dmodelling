...
# define model
formula <- as.formula(~ Habitat + Salinity + I(Salinity^3) + ReefMinDist + SuspendedParticulateMatter + 
                        I(SuspendedParticulateMatter^2) + NorthwardVelocity + TravelTime +
...
# outputs model
plotPars = list(GRIDPLOTS=T, SAVEPLOTS = T,  ncluster=3)
...
save(gjam, file="04_Modelling/01_benthic/02_eDNA/02_GJAM_eDNA/GJAM_Output_edna/gjam_model.rdata")


sens$variable <- rownames(sens)
sens <- sens[-16,]
sens$variable <- gsub("HabitatDeepSlope", "Habitat", sens$variable)
sens$group <- c(1,1,2,3,3,4,5,4,6,6,7,7,8,8,9)
...
ggsave(plot_sens, file="04_Modelling/01_benthic/02_eDNA/02_GJAM_eDNA/GJAM_Output_edna/gjamOutput/plot_sens.png", width=6, height = 5)
