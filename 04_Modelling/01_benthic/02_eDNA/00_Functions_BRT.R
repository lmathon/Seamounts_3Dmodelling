...
extract_best_parameters_par <- function(output, responseName_brts, distrib){
  ...
  write.csv(list(df = df),file=paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "train_results_", responseName_brts, "_", distrib, ".csv"))
...
fit_best_gaussian_brt_fixed <- function(data_brts, responseName_brts, best_params, myPredictor_brts){
  ...
  save(tbmod, file = paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "best_fixed_gaussian_brt_", responseName_brts, ".RData"))
...
fit_best_reduced_gaussian_brt_fixed <- function(data_brts, responseName_brts,  best_params, preds){
  ...
  save(tbmod, file = paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "best_fixed_reduced_gaussian_brt_", responseName_brts, ".RData"))
...
fit_best_reduced_gaussian_brt_gbmStep <- function(data_brts, responseName_brts,  best_params, preds){
  ...
  write.csv(list(BestModel = k.out),file=paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "final_results_mod_best_gbmStep_reduced_", responseName_brts, "_Ggaussian_", ".csv"))
  save(tbmod, file = paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "best_gbmStep_reduced_gaussian_brt_", responseName_brts, ".RData"))
...
make_contribution_reduced_plot <- function(model, responseName_brts, distrib){
  ...
  ggplot2::ggsave(paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", deparse(substitute(model)),"_", responseName_brts,"_", distrib , ".png"), plot, width = 6, height = 7)
  #write_csv(list(contrib = contrib),file=paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "contrib_",deparse(substitute(model)),"_", responseName_brts, "_", distrib, ".csv"))
  write_xlsx(list(contrib = contrib),path=paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "contrib_",deparse(substitute(model)),"_", responseName_brts, "_", distrib, ".xlsx"))
...
partial_dependance_plots3 <- function(model, responseName_brts, distrib){
  ...
  png(paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", "partial_plots3_", deparse(substitute(model)),"_", responseName_brts, "_", distrib, ".png"), width = width, height = height)
...
predict_brt <- function(model, distrib, species, preds, shp_rast){
  ...
  # Write raster
  if (distrib == "gaussian"){
    raster::writeRaster(p, here::here(paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", distrib, "_predictions_", species, ".grd")), overwrite=TRUE)
  } else { #exp transformation of predictions
    raster::writeRaster(exp(p), here::here(paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", distrib, "_predictions_", species, ".grd")), overwrite=TRUE)
  }
...
map_brt_prediction <- function(prediction, species, distrib){
  ...
  png(here::here(paste0("04_Modelling/01_benthic/02_eDNA/BRT_Output_edna/", distrib, "_map_brt_prediction_", species,".png")), width = 960, height = 480)
  if (distrib == "gaussian"){
