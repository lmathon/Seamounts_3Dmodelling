...
extract_best_parameters_par <- function(output, responseName_brts, distrib){
  ...
  write_xlsx(list(df = df),path=paste0("BRT_Output/", "train_results_", responseName_brts, "_", distrib, ".xlsx"))
...
fit_best_gaussian_brt_fixed <- function(data_brts, responseName_brts, best_params, myPredictor_brts){
  ...
  save(tbmod, file = paste0("BRT_Output/", "best_fixed_gaussian_brt_", responseName_brts, ".RData"))
...
fit_best_reduced_gaussian_brt_fixed <- function(data_brts, responseName_brts,  best_params, preds){
  ...
  save(tbmod, file = paste0("BRT_Output/", "best_fixed_reduced_gaussian_brt_", responseName_brts, ".RData"))
...
fit_best_reduced_gaussian_brt_gbmStep <- function(data_brts, responseName_brts,  best_params, preds){
  ...
  write_xlsx(list(BestModel = k.out),path=paste0("BRT_Output/", "final_results_mod_best_gbmStep_reduced_", responseName_brts, "_Gaussian_", ".xlsx"))
  save(tbmod, file = paste0("BRT_Output/", "best_gbmStep_reduced_gaussian_brt_", responseName_brts, ".RData"))
...
make_contribution_reduced_plot <- function(model, responseName_brts, distrib){
  ...
  ggplot2::ggsave(paste0("BRT_Output/", deparse(substitute(model)),"_", responseName_brts,"_", distrib , ".png"), plot, width = 6, height = 7)
  write_xlsx(list(contrib = contrib),path=paste0("BRT_Output/", "contrib_",deparse(substitute(model)),"_", responseName_brts, "_", distrib, ".xlsx"))
...
partial_dependance_plots3 <- function(model, responseName_brts, distrib){
  ...
  png(paste0("BRT_Output/", "partial_plots3_", deparse(substitute(model)),"_", responseName_brts, "_", distrib, ".png"), width = width, height = height)
...
predict_brt <- function(model, data_type, distrib, species, preds, shp_rast){
  ...
  # load mpa type and resample
  #mpa_type = mspproject::load_mpa_type_raster()
  #mpa_type = raster::resample(mpa_type, travel_time)
  ...
  #is.na(stackpreds[["dist_reef"]]) <- ! sel
